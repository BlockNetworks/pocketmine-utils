...
function read_PM_src($srcdir) {
	...
	if ($pm === null) die("$srcdir: Unable to read PocketMine file\n");
	...
	foreach (["VERSION","API_VERSION","CODENAME",
		...
		if (!isset($attr[$v])) die("Missing attribute $v\n");
	}
	// VERSION
	// API_VERSION
	// CODENAME
...
function exec_cmd($args) {
	...
	passthru($cmdline,$rv);
	if ($rv) die("Error executing command: $cmdline\n");
}
...
if (!FileUtils::cp_r($srcdir."src",$tmp."/src")) die("$tmp: copy error\n");
